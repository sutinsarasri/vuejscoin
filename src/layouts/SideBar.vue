<template>
  ...
  >
    ...
    <div class="collapse navbar-collapse w-auto" id="sidenav-collapse-main">
      <ul class="navbar-nav">
        <li v-for="(rowData, index) in listMenu" :key="index" class="nav-item">
          <a :class="['nav-link', rowData.active ? 'active' : '']" :href="rowData.path">
            <div
              class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center"
            >
              <span v-html="rowData.icon"></span>
            </div>
            <span class="nav-link-text ms-1">{{ rowData.name }}</span>
          </a>
        </li>
      </ul>
    </div>
